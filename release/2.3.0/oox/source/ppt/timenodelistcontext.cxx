 ...
 *  $Revision: 1.1.2.13 $
 *
 *  last change: $Author: hub $ $Date: 2007/07/12 20:00:57 $
 *
...
namespace oox { namespace ppt {
	...
	public:
		CmdTimeNodeContext( const FragmentHandlerRef& xHandler, sal_Int32  aElement,
			...
			: TimeNodeContext( xHandler, aElement, xAttribs, pNode )
				, maType(0)
			{
				OSL_TRACE( "OOX: CmdTimeNodeContext");
				switch ( aElement )
				{
	...
	public:
		SequenceTimeNodeContext( const FragmentHandlerRef& xHandler, sal_Int32  aElement,
			...
			: TimeNodeContext( xHandler, aElement, xAttribs, pNode )
				...
				, mnPrevAc(0)
			{
				OSL_TRACE( "OOX: SequenceTimeNodeContext::SequenceTimeNodeContext()" );
				AttributeList attribs(xAttribs);
		...
		virtual Reference< XFastContextHandler > SAL_CALL createFastChildContext( ::sal_Int32 aElementToken, 
			...
			throw ( SAXException, RuntimeException )
			{
				...
				case NMSP_PPT|XML_nextCondLst:
					xRet.set( new CondListContext( getHandler(), aElementToken, xAttribs, mpNode, maNextCondList ) );
					break;
				case NMSP_PPT|XML_prevCondLst:
					xRet.set( new CondListContext( getHandler(), aElementToken, xAttribs, mpNode, maPrevCondList ) );
					break;
				default:
					break;
				}
	...
	private:
		bool mbConcurrent;
		AnimationConditionList  maNextCondList, maPrevCondList;
	...
	public:
		ParallelExclTimeNodeContext( const FragmentHandlerRef& xHandler, sal_Int32  aElement,
			...
			: TimeNodeContext( xHandler, aElement, xAttribs, pNode )
			{
				OSL_TRACE( "OOX: ParallelExclTimeNodeContext::ParallelExclTimeNodeContext()" );
			}
		...
		virtual Reference< XFastContextHandler > SAL_CALL createFastChildContext( ::sal_Int32 aElementToken, const Reference< XFastAttributeList >& xAttribs ) throw ( SAXException, RuntimeException )
			{
					...
					// CT_TLByHslColorTransform
				{
					if( mbHasByColor )
					{
						...
						m_byColor.one = GetPercent( xAttribs->getOptionalValue( XML_h ) );
						m_byColor.two = GetPercent( xAttribs->getOptionalValue( XML_s ) );
						m_byColor.three = GetPercent( xAttribs->getOptionalValue( XML_l ) );
					}
					xRet.set(this);
					break;
				}
				case NMSP_PPT|XML_rgb:
				{
					if( mbHasByColor )
					{
						...
						m_byColor.one = GetPercent( xAttribs->getOptionalValue( XML_r ) );
						m_byColor.two = GetPercent( xAttribs->getOptionalValue( XML_g ) );
						m_byColor.three = GetPercent( xAttribs->getOptionalValue( XML_b ) );
					}
					xRet.set(this);
					break;
				}
				...
				case NMSP_PPT|XML_to:
					// CT_Color
					mbHasToClr = true;
					xRet.set( new colorChoiceContext( getHandler(), mnToClr ) );
					break;
				case NMSP_PPT|XML_from:
					// CT_Color
					mbHasFromClr = true;
					xRet.set( new colorChoiceContext( getHandler(), mnFromClr ) );
					break;
		...
		~AnimContext() throw ()
			{
				...
				if( nKeyTimes > 0)
				{
					...
					{
						aKeyTimes[i] = iter->msTime.toFloat();
...
//				OUString sPrList = xAttribs->getOptionalValue( XML_prLst );
	...
	TimeNodeContext * TimeNodeContext::makeContext( const FragmentHandlerRef& xHandler, sal_Int32  aElement, 
																									...
																									const TimeNodePtr & pNode )
	{
		OSL_TRACE( "OOX: TimeNodeContext::makeContext()" );
    ...
    TimeNodeContext::TimeNodeContext( const FragmentHandlerRef& xHandler, sal_Int32 aElement, 
		...
		, mpNode( pNode )
	{
		OSL_TRACE( "OOX: TimeNodeContext::TimeNodeContext()" );
	}
	...
	TimeNodeListContext::TimeNodeListContext( const FragmentHandlerRef& xHandler, TimeNodePtrList & aList )
		...
		: Context( xHandler )
			, maList( aList )
	{
		OSL_TRACE( "OOX: TimeNodeListContext::TimeNodeListContext()" );
	}
