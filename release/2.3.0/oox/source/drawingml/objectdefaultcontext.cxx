 ...
 *  $Revision: 1.1.2.2 $
 *
 *  last change: $Author: sj $ $Date: 2007/03/07 14:54:46 $
 *
...
objectDefaultContext::objectDefaultContext( const ::oox::core::FragmentHandlerRef& xHandler, ThemePtr pThemePtr )
: Context( xHandler )
...
Reference< XFastContextHandler > objectDefaultContext::createFastChildContext( sal_Int32 aElementToken, const Reference< XFastAttributeList >& /* xAttribs */ ) throw (SAXException, RuntimeException)
{
	...
	switch( aElementToken )
	{
		...
		{
			xRet.set( new spDefContext( getHandler(), mpThemePtr->getspDef() ) );
			break;
		}
		case NMSP_DRAWINGML|XML_lnDef:
		{
			xRet.set( new spDefContext( getHandler(), mpThemePtr->getlnDef() ) );
			break;
		}
		case NMSP_DRAWINGML|XML_txDef:
		{
			xRet.set( new spDefContext( getHandler(), mpThemePtr->gettxDef() ) );
			break;
		}
	}
	if( !xRet.is() )
