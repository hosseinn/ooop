	...
	throw (SAXException, RuntimeException)
{
	...
	switch( aElementToken )
	{
		case NMSP_VML|XML_imagedata:
			{
				rtl::OUString aRelId( xAttribs->getOptionalValue( NMSP_OFFICE|XML_relid ) );
				if ( aRelId.getLength() )
				{
					RelationPtr pRel( getHandler()->getRelations()->getRelationById( aRelId ) );
					if ( pRel )
						mpShapePtr->msGraphicURL = getHandler()->resolveRelativePath( pRel->msTarget );
				}
