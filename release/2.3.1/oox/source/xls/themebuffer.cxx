...
ThemeBuffer::ThemeBuffer( const GlobalDataHelper& rGlobalData ) :
...
::oox::drawingml::ThemePtr ThemeBuffer::getCoreTheme() const
{
    if( !mxTheme )
        mxTheme.reset( new ::oox::drawingml::Theme );
    return mxTheme;
}
...
{
    sal_Int32 nColor = 0;
    if( const ::oox::drawingml::ClrScheme* pClrScheme = getCoreTheme()->getClrScheme().get() )
...
sal_Int32 ThemeBuffer::getColorByIndex( sal_Int32 nIndex ) const
{
    ...
    sal_Int32 nColor = 0;
    if( const ::oox::drawingml::ClrScheme* pClrScheme = getCoreTheme()->getClrScheme().get() )
        if( (0 <= nIndex) && (nIndex < static_cast< sal_Int32 >( STATIC_TABLE_SIZE( spnColorTokens ) )) )
            if( pClrScheme->getColor( spnColorTokens[ nIndex ], nColor ) )
                return nColor;
    return API_RGB_TRANSPARENT;
}
...
{
    sal_Int32 nColor = 0;
    return ::oox::drawingml::ClrScheme::getSystemColor( nColor, nElement ) ? nColor : nDefaultColor;
}
