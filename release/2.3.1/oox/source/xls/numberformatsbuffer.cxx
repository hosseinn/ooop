...
{
public:
    explicit            NumberFormatFunctor( const GlobalDataHelper& rGlobalData );
...
NumberFormatFunctor::NumberFormatFunctor( const GlobalDataHelper& rGlobalData ) :
    maEnUsLocale( CREATE_OUSTRING( "en" ), CREATE_OUSTRING( "US" ), OUString() )
{
    try
    {
        Reference< XNumberFormatsSupplier > xNumFmtsSupp( rGlobalData.getDocument(), UNO_QUERY_THROW );
...
NumberFormat::NumberFormat( const GlobalDataHelper& rGlobalData ) :
...
void NumberFormat::setPredefinedId( const Locale& rLocale, sal_uInt16 nPredefId )
{
    maOoxData.maLocale = rLocale;
...
NumberFormatsBuffer::NumberFormatsBuffer( const GlobalDataHelper& rGlobalData ) :
    GlobalDataHelper( rGlobalData ),
    mnNextBiffIndex( 0 )
{
...
void NumberFormatsBuffer::importNumFmt( const AttributeList& rAttribs )
{
    sal_Int32 nNumFmtId = rAttribs.getInteger( XML_numFmtId, -1 );
    if( nNumFmtId >= 0 )
    {
        NumberFormatRef xNumFmt( new NumberFormat( getGlobalData() ) );
        maNumFmts[ nNumFmtId ] = xNumFmt;
        xNumFmt->setFormatCode( rAttribs.getString( XML_formatCode ) );
    }
}
...
void NumberFormatsBuffer::importFormat( BiffInputStream& rStrm )
{
    ...
    NumberFormatRef xNumFmt( new NumberFormat( getGlobalData() ) );
    maNumFmts[ mnNextBiffIndex ] = xNumFmt;
...
{
    maNumFmts.forEach( NumberFormatFunctor( getGlobalData() ) );
}
...
void NumberFormatsBuffer::insertBuiltinFormats()
{
    ...
    for( BuiltinVec::reverse_iterator aVIt = aBuiltinVec.rbegin(), aVEnd = aBuiltinVec.rend(); aVIt != aVEnd; ++aVIt )
    {
        ...
        for( const BuiltinFormat* pBuiltin = (*aVIt)->mpFormats; pBuiltin && (pBuiltin->mnNumFmtId >= 0); ++pBuiltin )
        {
            ...
            rxNumFmt.reset( new NumberFormat( getGlobalData() ) );
