...
WebQueryBuffer::WebQueryBuffer( const GlobalDataHelper& rGlobalData ) :
...
void WebQueryBuffer::dump() const
{
    ...
    fprintf(stdout, "----------------------------------------\n");
    {
        ...
        for (; itr != itrEnd; ++itr, ++nId)
        {
            if ( itr->mnType == Connection::CONNECTION_WEBQUERY )
            {
                ...
                fprintf(stdout, "WebQueryBuffer::dump: id = %ld  url = %s\n",
                        nId,
    ...
    for (; itr != itrEnd; ++itr)
    {
        fprintf(stdout, "WebQueryBuffer::dump: name = %s  connection ID = %ld\n",
                OUStringToOString(itr->first, RTL_TEXTENCODING_UTF8).getStr(), 
                itr->second.mnConnectionId);
    }
