...
# desired languages
setenv my_LANGUAGES "hu de fr it tr ka fi pl nl pt-BR es"

setenv BUILDNUMBER "OxygenOffice Professional 2.3.0 - OxygenOffice Build 2"

setenv BUILDER "OxygenOffice Professional Team"

./configure --with-lang="$my_LANGUAGES" --with-mspdb-path="$myMSPDB_PATH" --with-frame-home="$myFWSDK_PATH" --with-jdk-home="$myJSDK_PATH" --with-use-shell=tcsh --with-psdk-home="$myPSDK_PATH" --with-directx-home="$myDX_PATH" --with-ant-home=$myANTHOME --with-cl-home="$myCLHOME" --with-nmake-path="$myNMAKE_PATH" --with-csc-path="$myCSCPATH" --with-nsis-path="$myNSISPATH"  --with-build-version="$BUILDNUMBER" --with-vendor="$BUILDER" --disable-build-mozilla --enable-vba
