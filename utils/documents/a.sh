...
cp -f ./_TEMP_/* ./_TEMP_ML_/

echo [+] Compressing to OOOP-$folder-$languages-$version.oxt...
cd ./_TEMP_
zip -u -r -9 ../../output/OOOP-$folder-$languages-$version.oxt *
cd ..
...
echo [+] Remove OOOP-$folder-$version.oxt...
rm -f ../output/OOOP-$folder-multilanguage-$version.oxt

echo [+] Creating empty OOOP-$folder-multilanguage-$version.oxt...
cp -f MultiLanguageTemplatePackage.oxt ../output/OOOP-$folder-multilanguage-$version.oxt
